...
export default function DashboardPage() {
  const { stats, language, profile } = useApp()
  ...
  useEffect(() => {
    async function loadPlan() {
      try {
        const { data: res } = await courseAPI.getDailyPlan(profile?.level || 'Beginner', language)
  ...
  return (
    <div className="space-y-10">
      ...
      <div className="grid gap-6 sm:grid-cols-3">
        <Stat icon={Flame} label="XP Points" value={stats.xp} color="brand-cyan" />
        <Stat icon={Trophy} label="Study Points" value={stats.sp} color="brand-purple" />
        <Stat icon={Timer} label="Practice Streak" value={`${stats.streak} days`} color="brand-blue" />
      </div>
