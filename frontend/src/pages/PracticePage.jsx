      ...
      <div className="max-w-7xl mx-auto h-[calc(100vh-140px)] flex flex-col gap-8 relative z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div>
            ...
            <h1 className="text-4xl font-black text-white">Coding <span className="text-brand-cyan">Playground</span></h1>
          </div>

          <div className="flex items-center gap-6 bg-white/5 border border-white/5 p-4 rounded-3xl">
            <div className="text-right">
              <p className="text-[10px] font-bold text-white/30 uppercase tracking-widest mb-1">Progress</p>
              <p className="text-sm font-black text-white">{completedTasks.length} / {tasks.length} Tasks</p>
            </div>
            <div className="h-10 w-px bg-white/10"></div>
            <div className="text-right">
              <p className="text-[10px] font-bold text-white/30 uppercase tracking-widest mb-1">XP Reward</p>
        ...
        <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-8 min-h-0">
            ...
            <div className="glass-card p-8 rounded-[2.5rem] border-white/5 bg-mesh relative overflow-hidden">
              <div className="relative z-10">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <h3 className="text-xl font-bold text-white uppercase tracking-widest">Active Task</h3>
                </div>
                ...
                <AnimatePresence mode="wait">
                  ...
                  >
                    <p className="text-lg text-white/70 leading-relaxed font-medium">
                      {typeof tasks[activeTaskIndex]?.prompt === 'string' 
                        ? tasks[activeTaskIndex].prompt 
                        : (tasks[activeTaskIndex]?.prompt?.en || tasks[activeTaskIndex]?.prompt?.hi || 'Start coding to complete this task.')}
                    </p>
                    ...
                    <div className="p-4 rounded-2xl bg-brand-cyan/5 border border-brand-cyan/10 flex gap-3">
                      ...
                      <p className="text-xs text-brand-cyan/70 leading-relaxed italic">
                        {typeof tasks[activeTaskIndex]?.hint === 'string' 
                          ? tasks[activeTaskIndex].hint 
                          : (tasks[activeTaskIndex]?.hint?.en || 'Check the documentation if you get stuck!')}
                      </p>
                    </div>
                  </motion.div>
                </AnimatePresence>
                ...
                return (
                  ...
                  >
                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-xs font-black ${
                      isActive ? 'bg-brand-cyan text-bg-deep' : 'bg-white/5'
                    }`}>
                      {isCompleted ? <CheckCircle2 size={16} /> : i + 1}
                    </div>
                    <span className="text-sm font-bold truncate flex-1">
                      {String(typeof task.prompt === 'string' ? task.prompt : (task.prompt?.en || task.prompt?.hi || 'Untitled Task')).slice(0, 40)}...
                    </span>
          ...
          <div className="lg:col-span-8 flex flex-col h-full min-h-[500px]">
            <CodeRunner onRun={handleTaskComplete} />
          </div>
        </div>
      </div>
        ...
        {showReward && (
          ...
          >
            ...
            >
              ...
              <h2 className="text-3xl font-black text-white mb-4">Practice Complete!</h2>
              <p className="text-white/40 mb-8">You've successfully solved all practice tasks for this roadmap milestone.</p>
              
              <div className="flex items-center justify-center gap-4 mb-10">
                <div className="bg-white/5 px-6 py-3 rounded-2xl border border-white/5">
                  <p className="text-[10px] font-bold text-white/20 uppercase tracking-widest mb-1">Earned</p>
