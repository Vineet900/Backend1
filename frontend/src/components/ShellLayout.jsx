...
export default function ShellLayout() {
  ...
  const { assessmentMode } = useApp()

  const isEditorRoute = location.pathname.startsWith('/editor')
  const isChapterRoute = location.pathname.startsWith('/chapter/')

  return (
    <div className="min-h-screen bg-[#050816] text-white selection:bg-brand-cyan/30">
      {/* Dynamic Background */}
      ...
      {!isEditorRoute && !isChapterRoute && (
        <footer className="relative z-10 py-16 border-t border-white/5 bg-[#050816]/80 backdrop-blur-md">
          <div className="max-w-7xl mx-auto px-6">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-16 mb-16">
              ...
              {['Platform', 'Company'].map(group => (
                <div key={group}>
                  <h4 className="text-xs font-black text-white uppercase tracking-[0.2em] mb-8">{group}</h4>
