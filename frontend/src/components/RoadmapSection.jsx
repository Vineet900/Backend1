      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12 relative z-10">
        <div>
          ...
          <div className="flex items-center gap-4 mb-4">
            ...
            <h3 className="text-3xl md:text-4xl font-black text-white">{roadmap.title}</h3>
          </div>
  ...
  return (
    ...
    >
      <div className="relative flex flex-col items-center">
        ...
        {/* Card */}
        <div className={`w-full glass-card p-6 rounded-3xl border-white/10 group transition-all duration-500 hover:-translate-y-2 relative overflow-hidden ${step.isLocked ? 'opacity-50' : 'hover:border-brand-cyan/40 hover:shadow-[0_0_30px_rgba(34,211,238,0.1)]'}`}>
          {/* Locked Overlay */}
          {step.isLocked && (
          ...
          <div className="flex items-center justify-between mb-4">
            <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${step.isLocked ? 'bg-white/5' : 'bg-brand-cyan/10 text-brand-cyan'}`}>
              <Zap size={20} />
            </div>
          ...
          <h4 className="text-lg font-bold text-white mb-2 group-hover:text-brand-cyan transition-colors line-clamp-1">{step.title}</h4>
          <p className="text-xs text-white/40 leading-relaxed line-clamp-2 mb-6">{step.description}</p>

          <div className="flex items-center justify-between pt-4 border-t border-white/5">
            <div className="flex items-center gap-1.5">
              <Star size={12} className="text-brand-purple" />
              <span className="text-[10px] font-bold text-white/60">{step.xp} XP</span>
            </div>
            
            {!step.isLocked ? (
              <Link to={`/courses`} className="text-[10px] font-black text-brand-cyan uppercase tracking-widest flex items-center gap-1 hover:gap-2 transition-all">
